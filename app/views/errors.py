"""How a failure is rendered.

The exceptions themselves live in ``core.errors`` — a service raising
``NotFoundError`` is stating a fact about the domain, not choosing a status
code or a response body. Turning one into JSON is a view's job, and it happens
only here.
